...
export class RolebindingService {
  ...
  public listForUser(user: string): Observable<RoleBinding[]> {
    return this.http
      .get<ServerResponse>(
        environment.server + '/a/user/' + user + '/rolebindings',
      )
      .pipe(
  ...
  public list(): Observable<RoleBinding[]> {
    return this.http
      .get<ServerResponse>(environment.server + '/a/rolebindings/list')
      .pipe(
        switchMap((s: ServerResponse) => {
          return of(JSON.parse(atou(s.content)));
        }),
  ...
  public get(id: string): Observable<RoleBinding> {
    return this.http
      .get<ServerResponse>(environment.server + '/a/rolebindings/' + id)
      .pipe(
        switchMap((s: ServerResponse) => {
          return of(JSON.parse(atou(s.content)));
        }),
  ...
  public create(roleBinding: RoleBinding): Observable<boolean> {
    return this.http
      .post<ServerResponse>(
        environment.server + '/a/rolebindings/create',
        roleBinding,
      )
      .pipe(
        switchMap((s: ServerResponse) => {
  ...
  public update(roleBinding: RoleBinding): Observable<boolean> {
    return this.http
      .put<ServerResponse>(
        environment.server + '/a/rolebindings/' + roleBinding.name,
        roleBinding,
      )
      .pipe(
        switchMap((s: ServerResponse) => {
  ...
  public delete(id: string): Observable<boolean> {
    return this.http
      .delete<ServerResponse>(environment.server + '/a/rolebindings/' + id)
      .pipe(switchMap((s: ServerResponse) => of(s.message == 'deleted')));
  }
}
