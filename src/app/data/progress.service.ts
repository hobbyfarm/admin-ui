...
export class ProgressService {
  ...
  public listByRange(from: Date, to: Date) {
    const fromDateString: string = formatDate(
      from,
  ...
  public listByScheduledEvent(seId: string, includeFinished: boolean = false) {
    return this.gargAdmin
      .get('/scheduledevent/' + seId + '?includeFinished=' + includeFinished)
      .pipe(
        map(extractResponseContent),
        map((pList: Progress[]) => {
          return this.buildProgressList(pList);
  ...
  public count() {
    return this.gargAdmin.get('/count').pipe(map(extractResponseContent));
  }
